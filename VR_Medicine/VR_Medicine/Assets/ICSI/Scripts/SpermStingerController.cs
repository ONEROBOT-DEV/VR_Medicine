...
public class SpermStingerController : MonoBehaviour
{
    ...
    public void StartMoveToOutStinger()
    {
        ...
        if ((targetSperm.position - stingerPoint.position).magnitude < .02f)
        {
            onSpermOutStinger.Invoke();
            Debug.Log("sperm in a egg");
            targetSperm.SetParent(egg);
            isOutOnStinger = true;
            targetSperm.gameObject.SetActive(false);
        }
    }
    ...
    private void AnimationVacuum()
    {
        ...
        targetSperm.DORotate(Vector3.up * -90f, duration);
        duration += (stingerPoint.position - stingerTargetMovePoint.position).magnitude;
        targetSperm.DOLocalPath(new[] { stingerPoint.localPosition, stingerTargetMovePoint.localPosition },
            duration / speedVacuum).OnComplete(() => { onSpermInStinger?.Invoke(); });
    }
}