...
public class InputFieldScript : MonoBehaviour
{
    ...
    public string keyboardTag; // Keyboard tag (You need to set the tag of the keyboard you want to use on this InputField)

    KeyboardManager targetKeyboard; // Reference to keyboard manager
    ...
    {
        var targetKeyboardObj = GameObject.FindGameObjectWithTag(keyboardTag);
        if (targetKeyboardObj != null)
        {
            if (targetKeyboardObj.GetComponent<KeyboardManager>())
            {
                targetKeyboard = targetKeyboardObj.GetComponent<KeyboardManager>();
                targetKeyboard.SelectInput(this);
            }
            else
            {
                print("You have an object with a tag " + keyboardTag + " that is not a keyboard");
            }
        }
        else
        {
            print("You don't have an object with a tag " + keyboardTag);
        }
    }
    ...
    {
        targetKeyboard.DeselectInput(this);
    }
