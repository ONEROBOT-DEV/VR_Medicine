...
namespace TVP
{
    public class DeviceToVirtualNiddle : MonoBehaviour
    {
        ...
        public void SyncPos()
        {
            ...
            x = transform.localEulerAngles.x + InputHandlerOfDream.CurrentSimulation.Primary2DAxis.y * 3;
            y = transform.localEulerAngles.y + InputHandlerOfDream.CurrentSimulation.Primary2DAxis.x * 3;


            transform.DORotate(new Vector3(x, y, 0), 1);

            var input = Mathf.Clamp01(InputHandlerOfDream.CurrentSimulation.Secondary2DAxis.y);

            input = (input * 2 - 1);

              

            _niddleShape.transform.DOLocalMoveZ(input, 1);
        ...
        public void DoAspirate()
        {
            ...
            _biopsyAdapter.OnAspirate?.Invoke(PompController.CurrentSimulation.TotalPressaure / 10000);
        }
