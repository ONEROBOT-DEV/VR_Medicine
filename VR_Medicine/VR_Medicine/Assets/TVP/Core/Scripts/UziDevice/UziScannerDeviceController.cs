...
namespace TVP
{
    public class UziScannerDeviceController : InteractableBase
    {
        ...
        [SerializeField] Transform _vaginaPlace;
        ...
        {
            TVPTutorialAdapter.CurrentSimulation.CustomSticksTutorial();
            ...
            transform.position = _vaginaPlace.position;
            transform.eulerAngles = _vaginaPlace.eulerAngles;

            _rigidbody.velocity = Vector3.zero;
            _rigidbody.angularVelocity = Vector3.zero;
            _rigidbody.constraints = RigidbodyConstraints.FreezeAll;

            if (_uziDevice.Adapter != null)
            {
                _uziDevice.Adapter.gameObject.GetComponent<Rigidbody>().velocity = Vector3.zero;
                _uziDevice.Adapter.gameObject.GetComponent<Rigidbody>().angularVelocity = Vector3.zero;
            }

            if (_uziDevice.Niddle != null)
            {
                _uziDevice.Niddle.gameObject.GetComponent<Rigidbody>().velocity = Vector3.zero;
                _uziDevice.Niddle.gameObject.GetComponent<Rigidbody>().angularVelocity = Vector3.zero;
            }
        }
        ...
        public void DemountedFromVagine()
        {
            ...
            PlayerMovemetnManager.CurrentSimulation.RealesePlayer(); 
            _rigidbody.constraints = RigidbodyConstraints.None;
        }
