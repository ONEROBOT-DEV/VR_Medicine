...
namespace TVP
{
    ...
    public class SimDomenStateMachine : MonoSinglethon<SimDomenStateMachine>
    {
        ...
        internal DataContainer Container { get => _container; set => _container = value; }
        public SimDomenState CurrentState { get => _currentState; set => _currentState = value; }

        private void Awake()
        {
            STATE_TRANSITION = Common.DataManager.SimulationManagerDataContainer.TVPTest ? 0 : 1;
        ...
        {
            UnityAction resetScene = () => { ResetCurrentScene(); ErrorsManager.CurrentSimulation.LevelIsPassedWithoutError(CurrentState.CurrentLevelNum); };
            ErrorsManager.CurrentSimulation.LevelIsPassedWithError(CurrentState.CurrentLevelNum, "Вы не успели " + CurrentState.LevelTittle, resetScene, null, "Вы не успели " + CurrentState.LevelTittle, "Еще раз", "Продолжить");
        }
        ...
        private void Start()
        {
            ...
            if (Common.DataManager.SimulationManagerDataContainer.TVPTest)
                Test();
        ...
        public void ChangeState(SimDomenState newState)
        {
            ...
            if (CurrentStateNum >= CorrectSequence.Count)
                return;


            if (CorrectSequence[CurrentStateNum] != newState.StateType)
            {

                print($"STATE CAHNGE OCCURED [<color=red> {newState.StateType} </color>] BUT SHOULD BE [<color=green> {CorrectSequence[CurrentStateNum].ToString()} </color>]");
                return;
            }
            else
                CurrentStateNum++;
            ...
            print($"PREV. STATE[<color=orange> {newState.StateType} </color>] -> NEW STATE[<color=green> {CorrectSequence[CurrentStateNum].ToString()} </color>]");

            if (CurrentState != null)
            {
                Common.DataManager.SimulationStaticDataManager.FinishLevel(CurrentState.CurrentLevelNum);
            ...
            if (CurrentState != null)
            {
                ...
                Common.DataManager.SimulationStaticDataManager.StartLevel(CurrentState.CurrentLevelNum);
                SimStateCanvas.CurrentSimulation.UpdateUI(CurrentState);
            }
        }
        ...
        IEnumerator DelayedStateAdapter(StateTypeEnum state)
        {
            ...
            yield return new WaitForSeconds(STATE_TRANSITION);
        ...
        {
            CurrentStateNum = CorrectSequence.IndexOf(state);
            StateAdapter(state);
        }
